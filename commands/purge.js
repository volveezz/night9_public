        ...
        if (user) {
            const fetched = yield ((_a = interaction.channel) === null || _a === void 0 ? void 0 : _a.messages.fetch({ limit: msgs }).then((response) => {
                return response
                    ...
                    .filter((msg) => msg.createdTimestamp >
                    new Date().getTime() - 60 * 1000 * 60 * 24 * 14);
            }));
            interaction.channel
                .bulkDelete(fetched)
                .then((response) => {
            ...
            const msgArray = yield interaction.channel.messages
                .fetch({ limit: msgs })
                .then((m) => {
                return m.filter((msg) => msg.createdTimestamp >
                    new Date().getTime() - 60 * 1000 * 60 * 24 * 14);
            });
            interaction.channel
                .bulkDelete(msgArray)
                ...
                const embed = new discord_js_1.EmbedBuilder()
                    .setColor("Green")
                    .setTitle(`${response.size} сообщений были удалены`);
                interaction.editReply({ embeds: [embed] });
                return;
            })
                .catch((e) => {
