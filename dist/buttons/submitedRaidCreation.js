...
const ButtonCommand = new Button({
    ...
    run: async ({ modalSubmit: interaction, client }) => {
        ...
        const raidNameInput = (modalFields.getTextInputValue("RaidNameField") || "").replaceAll(/\s\s+/, " ");
        const raidTimeInput = (modalFields.getTextInputValue("RaidTimeField") || "").replaceAll(/\s\s+/, " ");
        const raidDescriptionInput = (modalFields.getTextInputValue("RaidDescriptionField") || "").replaceAll(/\s\s+/, " ");
        const raidClearRequirementInput = +(modalFields.getTextInputValue("RaidClearRequirementField") || "0").replaceAll(/\s\s+/, " ") || 0;
