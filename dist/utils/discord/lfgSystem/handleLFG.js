...
function getTextInputValue(interaction, fieldName) {
    return (interaction.fields.getTextInputValue(fieldName) || "")?.replace(/\s\s+/g, " ");
}
async function handleLfgModal(interaction) {
    ...
    if (roomActivityName.length < 1)
        return resolveLfgError("Ошибка названия", "Слишком короткое или пустое", interaction);
    ...
    if (userLimitString.startsWith("+"))
        userLimit + 1;
    if (userLimit <= 0 || userLimit >= 98)
        userLimit = 99;
    await createLfgPost({
        roomActivityName,
        userLimit,
...
async function handleLfgMessage(message) {
    ...
    const roomActivityName = userMessageContent
        ...
        .trim()
        .replace(/^в\s*/, "");
    const description = separatorIndex !== -1 ? userMessageContent.slice(userMessageContent.indexOf("|") + 1, userMessageContent.length).trim() : null;
...
async function createLfgPost({ userLimit, roomActivityName, description, additionalParams, activityName, message, user, interaction, }) {
    if (roomActivityName.length < 1 || isNaN(userLimit) || userLimit <= 1 || userLimit > 99) {
        const errorMsg = roomActivityName.length < 1
    ...
    try {
        ...
        embed.setTitle(`+${userLimitWithCurrentVoiceMembers < 0 ? 0 : userLimitWithCurrentVoiceMembers} в ${(userSettings.activitySettings?.name ?? roomActivityName) || "неизвестную активность"}`);
    }
    catch (error) {
    ...
    if (member) {
        embed.addFields({
            ...
            value: `<@${user.id}> — ${nameCleaner(member.displayName, true)}${bungieNames.has(user.id) ? ` — ${escapeString(bungieNames.get(user.id))}` : ""}`,
        });
    }
    ...
    if (!voiceChannel)
        return resolveLfgError("Ошибка во время создания голосового канала", "Попробуйте позже", message || interaction);
    ...
    const ping = userSettings.ping && !checkIfUserRecentlyCreatedActivity(user.id)
...
async function resolveLfgError(name, description, input) {
