...
const createFieldValue = (message: Message<boolean> | PartialMessage) => {
	let fieldValue = "";
	...
	if (message.attachments.size > 0 && fieldValue.length < 990) {
		...
		message.attachments.forEach((attachment) => {
			if (fieldValue.length + attachment.name.length + attachment.url.length > 1020) return;
...
const getTotalEmbedsSize = (embeds: EmbedBuilder[]) => {
