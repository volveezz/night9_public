...
let voiceLogChannel: TextChannel | null = null;
...
export default new Event("voiceStateUpdate", async (oldState, newState) => {
	...
	if (!oldState.channelId && newState.channelId) {
		...
		embed
			.setAuthor({
				name: `${oldState.member?.displayName || newState.member?.displayName || "пользователь не найден"} присоединился к ${
	...
	if (!newState.channelId) {
		...
		embed
			.setAuthor({
				name: `${oldState.member?.displayName || newState.member?.displayName || "пользователь не найден"} вышел из ${
	...
	if (oldState.channelId && newState.channelId && oldState.channelId !== newState.channelId) {
		if (oldState.member && channelDataMap.has(oldState.channelId)) lfgTextChannelHandler(oldState.channelId, oldState.member, "leave");
