...
function isValidMessage(message: any): message is { author: { id: string } } {
	if (
		message.system ||
		!message.author ||
...
function createDeletedMessageEmbed(message: Message) {
...
async function getDeleterFromAuditLogs(message: Message, embed: EmbedBuilder) {
...
export default new Event("messageDelete", async (message) => {
	...
	if (!messageChannel) messageChannel = await client.getTextChannel(process.env.MESSAGES_CHANNEL_ID!);
	...
	async function processAttachments() {
		...
		// Loop through all attachments and upload them to Imgur.
		const uploadPromises = message.attachments.map(async (msgAttachment) => {
			try {
				...
				// Create a new embed for the uploaded image and push it into embeds array.
			} catch (error) {
	...
	function processStickers() {
		const stickerNamesArray: string[] = [];
