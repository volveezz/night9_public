import { AuditLogEvent, Collection, EmbedBuilder, GuildMember, Role, TextChannel } from "discord.js";
...
export default new Event("guildMemberUpdate", async (oldMember, newMember) => {
	if (!oldMember.joinedTimestamp || (!oldMember.nickname && oldMember.roles.cache.size === 0)) return;

	const embeds: EmbedBuilder[] = [];

	const rolesEmbed = await generateRoleEmbed(oldMember, newMember);
	if (rolesEmbed) embeds.push(rolesEmbed);

	const nameEmbed = await generateNameEmbed(oldMember, newMember);
	if (nameEmbed) embeds.push(nameEmbed);

	const muteEmbed = generateMuteEmbed(oldMember, newMember);
	if (muteEmbed) embeds.push(muteEmbed);

	if (embeds.length > 0) {
		if (!guildMemberChannel) guildMemberChannel = await client.getTextChannel(process.env.GUILD_MEMBER_CHANNEL_ID!);
		await guildMemberChannel.send({ embeds });
	}
});

async function generateRoleEmbed(oldMember: GuildMember, newMember: GuildMember) {
	const oldRoles = oldMember.roles.cache;
	const newRoles = newMember.roles.cache;
	const addedRoles = newRoles.filter((role) => !oldRoles.has(role.id));
	const removedRoles = oldRoles.filter((role) => !newRoles.has(role.id));

	if (addedRoles.size === 0 && removedRoles.size === 0) return null;
...
async function checkNameExecutor(member: GuildMember): Promise<GuildMember | null> {
	...
	if (!nicknameChange) return member;

	const { executor } = nicknameChange;

	if (!executor) return null;
	if (executor.id === member.id) return null;
...
async function generateNameEmbed(oldMember: GuildMember, newMember: GuildMember) {
...
function generateMuteEmbed(oldMember: GuildMember, newMember: GuildMember) {
	if (oldMember.communicationDisabledUntilTimestamp === newMember.communicationDisabledUntilTimestamp) return null;
...
function addRolesField(roles: Collection<string, Role>, singularTitle: string, pluralTitle: string, embed: EmbedBuilder) {
...
async function testAutonameUserStatus(member: GuildMember) {
	if (notifiedUsers.has(member.id)) return;

	const authData = await AuthData.findOne({ where: { discordId: member.id }, attributes: ["displayName", "timezone"] });

	if (!authData || authData.displayName.startsWith("⁣")) return;

	function checkUserName(databaseName: string, displayName: string, timezone: number | null | undefined) {
		const expectedName = timezone !== null ? `[+${timezone}] ${databaseName}` : databaseName;
		return displayName === expectedName;
	}

	if (checkUserName(authData.displayName, member.displayName, authData.timezone)) return;
	...
	const embed = new EmbedBuilder()
		...
		// .setDescription(
		// 	`На сервере работает система слежки за никнеймами пользователей в игре\n\n> Ваш никнейм в игре: \`${escapeString(
		// 		authData.displayName
		// 	)}\`\n> Текущий никнейм: \`${escapeString(member.displayName)}\`${`\n> Текущий часовой пояс: ${
		// 		authData.timezone ? `\`${authData.timezone}\`` : `не указан. Введите \`/timezone\` (</timezone:1055308734794043503>)`
		...
		.setDescription(
			`На сервере включена система отслеживания игровых никнеймов.\n\n- Ваш игровой никнейм: \`${escapeString(
				authData.displayName
