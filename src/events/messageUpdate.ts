...
export default new Event("messageUpdate", async (oldMessage, newMessage) => {
	if (newMessage.content === "[Original Message Deleted]") newMessage.delete().catch((_) => null);
	if (!oldMessage.content?.length || oldMessage.content === newMessage.content || newMessage.author?.id === client.user.id) return;
		...
		? embed.addFields(
				{
					name: "До изменения",
					value:
						!oldMessage.content || oldMessage.content.length === 0 ? "сообщение не было в кеше" : escapeString(oldMessage.content),
				},
