import { ButtonBuilder, ButtonComponent, ButtonStyle, ColorResolvable, EmbedBuilder, Role, TextChannel, VoiceChannel } from "discord.js";
	...
	run: async ({ client, interaction }) => {
		...
		switch (categoryId) {
				...
				collector.on("collect", async (m) => {
					...
					const role = member.roles.highest.name.startsWith("◈")
						...
						: await member.roles.add(
								await guild.roles.create({ name: `◈ ${name}`, position: member.roles.highest.position + 1 }).catch((e) => {
									throw { name: "Проверьте название роли" };
								})
						  );
			...
			case "color": {
				...
				if (member.roles.highest.name === "⁣") await member.roles.remove(member.roles.highest);
		...
		async function sortRaids() {
			...
			const currentRaids = (await RaidEvent.findAll({ where: { time: { [Op.gt]: Math.floor(Date.now() / 1000) } } })).sort((a, b) => {
				// If raid is full, it should appear at the beginning.
				if (a.joined.length >= 6 && b.joined.length < 6) return -1;
				if (b.joined.length >= 6 && a.joined.length < 6) return 1;

				// If both are full or both are not full, sort them by time.
				if (a.time !== b.time) return b.time - a.time;

				// If times are equal, and both are either full or not full, sort them by the length of joined.
			...
			if (!raidChannel) raidChannel = await client.getTextChannel(process.env.RAID_CHANNEL_ID!);
			...
			for (const raid of currentRaids) {
				try {
					...
					const components = message.components
						.map((c) =>
							c.components.map((com) => {
								const button = com as ButtonComponent;
					...
					const updatedRaidMessage = await raidChannel.send({
						...
						components: addButtonsToMessage(
							components && components.length > 0 ? components : subComponents ? subComponents : baseComponents
						),
					});
		...
		async function customRoleColor() {
			...
			collector.on("collect", async (m) => {
				...
				if (code.length > 9) throw { errorType: UserErrors.WRONG_HEX };

				const role = member.roles.highest.name.startsWith("◈")
					? await member.roles.highest.edit({ color: code as ColorResolvable }).catch((e) => {
							throw { errorType: UserErrors.WRONG_HEX };
					  })
					: await member.roles.add(
							await guild.roles
								.create({ name: "◈", color: code as ColorResolvable, position: member.roles.highest.position + 1 })
								.catch((e) => {
									throw { errorType: UserErrors.WRONG_HEX };
								})
					  );
		...
		function chnPermGranter(chnId: string) {
