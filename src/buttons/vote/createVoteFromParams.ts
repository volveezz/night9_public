...
import { ButtonBuilder, ButtonStyle, ChannelType, ComponentType, EmbedBuilder, ModalSubmitInteraction } from "discord.js";
...
async function createVoteFromParams({ interaction, question, description, answers, image }: VoteCreationParams) {
	...
	collector.on("collect", (buttonInteraction) => {
		...
		async function editVote() {
			...
			validatedQuestion = modalReply.fields.getField(VoteModal.ModifyVoteQuestion, ComponentType.TextInput).value;
			validatedDescription = modalReply.fields.getField(VoteModal.ModifyVoteDescription, ComponentType.TextInput).value;
			validatedAnswers = modalReply.fields.getField(VoteModal.ModifyVoteAnswers, ComponentType.TextInput).value.split(" | ");
			imageUrl = modalReply.fields.getField(VoteModal.ModifyVoteImage, ComponentType.TextInput).value;
		...
		async function sendVote() {
			...
			// Remove the last field, which is the answers field
			...
			const query = VotingDatabase.create({
				...
				VoteSystem.getInstance().addVote(uniqueId, allowMultipleAnswers, message.id, interaction.user.id, message.channelId)
			);

			await Promise.all([message, query]);

			function generateComponents(fieldTitles: string[]) {
				const uniqueId = randomUUID().split("-")[0];
				const returnValues: { uniqueId: string; components: ButtonBuilder[] } = {
					uniqueId,
					components: [],
				};
	...
	collector.on("end", (c, r) => {
		if (r === "time") reply.edit({ components: [], content: "Время ожидания истекло. Вы можете скопировать текст из сообщения ниже" });
	});
}
