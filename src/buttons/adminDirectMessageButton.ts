...
export const workingCollectors = new Map<string, MessageCollector>();

const ButtonCommand = new Button({
	name: "adminDirectMessageButton",
	run: async ({ client, interaction }) => {
		if (interaction.customId !== AdminDMChannelButtons.delete && interaction.customId !== AdminDMChannelButtons.reply) return;
		...
		switch (customId) {
			case AdminDMChannelButtons.reply: {
				...
				collector.on("collect", async (m) => {
					m.delete();
					if (m.cleanContent === "cancel") return collector.stop("canceled");
					const contentText = descriptionFormatter(
						m.cleanContent.endsWith("embed") && m.cleanContent.length > 5 ? m.cleanContent.slice(0, -5) : m.cleanContent
					);
					const replyContent =
						m.cleanContent.endsWith("embed") && m.cleanContent.length > 5
				...
				collector.on("end", (_, reason) => {
					if (reason === "canceled") interaction.deleteReply();
					workingCollectors.delete(interaction.user.id);
				});
				return;
			}
