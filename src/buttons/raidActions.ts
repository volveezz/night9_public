...
const raidGuideSentUser = new Map<RaidNames, Set<string>>();
...
const ButtonCommand = new Button({
	...
	run: async ({ client, interaction }) => {
		...
		if (raidData.requiredRole && member.roles.cache.has(process.env.VERIFIED!) && !member.roles.cache.has(raidData.requiredRole)) {
			await deferredUpdate;
			throw { errorType: UserErrors.ACTIVITY_MISSING_DLC, errorData: [raidData.requiredRole] };
		}
		...
		const userTarget =
			interaction.customId === RaidButtons.join
		...
		if (interaction.customId === RaidButtons.join) {
			...
			const sentUserSet = raidGuideSentUser.get(raidEvent.raid) ?? new Set<string>();
		...
		await (
			await client.getTextChannel(raidEvent.channelId)
		).permissionOverwrites.create(interaction.user.id, {
			ViewChannel: true,
		});

		if (interaction.customId === RaidButtons.join && userTarget === "joined") {
