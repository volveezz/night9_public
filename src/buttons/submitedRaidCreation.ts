...
const ButtonCommand = new Button({
	...
	run: async ({ modalSubmit: interaction, client }) => {
		...
		const raidNameInput = (modalFields.getTextInputValue(RaidCreation.RaidNameField) || "").replace(/\s\s+/g, " ");
		const raidTimeInput = (modalFields.getTextInputValue(RaidCreation.RaidTimeField) || "").replace(/\s\s+/g, " ");
		const raidDescriptionInput = (modalFields.getTextInputValue(RaidCreation.RaidDescriptionField) || "").replace(/\s\s+/g, " ");
		const raidClearRequirementInput =
			+(modalFields.getTextInputValue(RaidCreation.RaidClearRequirementField) || "0").replace(/\s\s+/g, " ") || 0;
...
function validateRaidName(input: string): RaidNames {
	switch (input) {
...
function findRaidDifficulty(input: string): RaidDifficulty {
	switch (input) {
		case input.match(/(master|мастер)/i)?.input:
			return RaidDifficulty.Master;
		default:
			return RaidDifficulty.Normal;
	}
}

function validateRaidTime(input: string) {
...
function validateRaidDescription(input: string) {
