import { LFGButtons } from "../configs/LFGButtons.js";
...
const ButtonCommand = new Button({
	...
	run: async ({ interaction }) => {
		...
		if (!lfgId) {
			...
			throw { errorType: UserErrors.LFG_NOT_FOUND, interaction, deferred: deferUpdatePromise };
		}

		switch (true) {
			case customId.startsWith(LFGButtons.join): {
			...
			case customId.startsWith(LFGButtons.leave): {
				LFGController.getInstance()
