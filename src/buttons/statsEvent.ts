import { DestinyMilestone, DestinyProfileResponse } from "bungie-api-ts/destiny2/interfaces.js";
import { PlatformErrorCodes } from "bungie-api-ts/groupv2";
...
const ButtonCommand = new Button({
	...
	run: async ({ interaction }) => {
		if (interaction.customId !== StatsButton.oldEvents && interaction.customId !== StatsButton.pinnacle) return;
		...
		switch (interaction.customId) {
			case StatsButton.oldEvents: {
				const data = await sendApiRequest<DestinyProfileResponse>(
					`/Platform/Destiny2/${platform}/Profile/${bungieId}/?components=202`,
					userData
				);
				...
				const embedFields = dataFact.slice(0, 25).map(({ factionHash, progressionHash, currentProgress, level, levelCap }) => {
					const embedName =
						progressionDefinition[progressionHash]?.displayProperties?.name ||
			...
			case StatsButton.pinnacle: {
				const data = await sendApiRequest<DestinyProfileResponse>(
					`/Platform/Destiny2/${platform}/Profile/${bungieId}/?components=200,202`,
					userData
				);
				...
				if (!embedDescription) embedDescription = "Персонажи отсутствуют";
				...
				collector.on("collect", async (collected) => {
					...
					const storedMilestones: {
						milestoneHash: DestinyMilestone["milestoneHash"];
						endDate: number;
						rewards: DestinyMilestone["rewards"];
					}[] = [];

					Object.entries(characterMilestones).forEach(([k, milestone]) => {
						if (!milestone.rewards || milestone.rewards[0].rewardCategoryHash !== 326786556) return;
					...
					storedMilestones.forEach((mile) => {
						if (curDate > mile.endDate) return;

						mile.rewards.forEach((reward) => {
							reward.entries.forEach((subRew) => {
								if (subRew.redeemed === true || (embed.data.fields && embed.data.fields.length >= 25)) return;
