import { ButtonBuilder, ButtonComponent, ButtonInteraction, ButtonStyle, ComponentType, Message } from "discord.js";
import { RaidButtons } from "../../configs/Buttons.js";
import { client } from "../../index.js";
import { RaidEvent } from "../../utils/persistence/sequelizeModels/raidEvent.js";

type UnlockRaidMessageParams = {
	interaction: ButtonInteraction;
	raidEvent: RaidEvent;
};

async function unlockRaidMessage({ interaction, raidEvent }: UnlockRaidMessageParams) {
	const raidChannel = client.getCachedTextChannel(process.env.RAID_CHANNEL_ID!);
	const raidMessage =
		raidChannel.messages.cache.get(raidEvent.messageId) || (await client.getAsyncMessage(raidChannel, raidEvent.messageId));
...
async function raidButtonsUnlocker({ interaction, raidEvent }: UnlockRaidMessageParams, raidMessage: Message) {
	const inChannelMessageButtonRows = interaction.message.components.map((actionRow) => {
		const inChannelMessageButtons = actionRow.components.map((component) => {
			const unlockButton = component as ButtonComponent;
