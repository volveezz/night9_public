...
async function updateFireteamNotification(raidEventOrId: RaidEvent | number, isDisabling: boolean) {
	const raidEvent =
		typeof raidEventOrId === "number" ? await RaidEvent.findByPk(raidEventOrId, { attributes: ["channelId", "id"] }) : raidEventOrId;
	...
	if (isDisabling) {
		...
		fireteamNotificationComponent = new ButtonBuilder()
			.setCustomId(RaidButtons.fireteamTrackerStart)
			.setLabel("Запустить")
			.setStyle(ButtonStyle.Success);
	} else {
		fireteamNotification = new EmbedBuilder().setColor(colors.default).setTitle(ENABLED_TEXT);
		fireteamNotificationComponent = new ButtonBuilder()
			.setCustomId(RaidButtons.fireteamTrackerCancel)
			.setLabel("Остановить")
			.setStyle(ButtonStyle.Danger);
	}

	const notificationMessage = privateRaidChannel.messages.cache.find(
		(m) => m.embeds?.length !== 0 && (m.embeds[0].title === ENABLED_TEXT || m.embeds[0].title === DISABLED_TEXT)
	);
