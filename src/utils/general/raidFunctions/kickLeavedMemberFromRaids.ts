import { EmbedBuilder, GuildBan, GuildMember, PartialGuildMember } from "discord.js";
...
export default async function kickLeavedUserFromRaids(member: GuildMember | GuildBan | PartialGuildMember) {
	...
	if (rowsUpdated > 0) {
		raidEvents.forEach(async (raidEvent) => {
			try {
				if (raidEvent.creator === userId) await transferRaidCreator(raidEvent);
			} catch (error: any) {
