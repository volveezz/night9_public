...
export async function processVexIncursionMessage(message: Message) {
	if (message.author.id !== "1096872850255794176") return;
...
export async function removeNewsChannelOriginalButtons() {
	...
	const messageWithButtons = messages.filter(
		(m) => m.components?.[0]?.components?.[0]?.customId === TwitterButtons.showOriginal && m.createdTimestamp < client.readyTimestamp
	);
