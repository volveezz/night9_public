import { EmbedBuilder, GuildMember, Message, RESTJSONErrorCodes } from "discord.js";
...
const lastOpenedDM = new Map<string, GuildMember>();

async function sendDirectMessage(member: GuildMember, isEmbed: boolean, content: string, originatingMessage: Message<boolean>) {
	try {
		...
		const errorEmbed = new EmbedBuilder()
			...
			.setAuthor({
				name: `Пытались отправить: ${member.displayName || member.user.username}${
...
export async function manageAdminDMChannel(message: Message) {
	...
	if (!isSnowflake(userId) && !lastDM) return;

	const member =
		(isSnowflake(userId) && !lastDM) || (isSnowflake(userId) && userId !== lastDM?.id) ? await client.getMember(userId) : lastDM;
