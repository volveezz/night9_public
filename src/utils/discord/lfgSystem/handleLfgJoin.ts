...
export async function removeChannelData(data: string | LfgEvent) {
	...
	if (channelData.isDeletable) {
		try {
			const userVoiceCount = ((await client.getCachedGuild().channels.fetch(channelId)) as VoiceChannel)?.members.size;
...
async function handleTextChannel(channelId: string, member: GuildMember, action: "join" | "leave") {
