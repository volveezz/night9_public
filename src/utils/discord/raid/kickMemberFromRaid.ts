...
type KickMemberFromRaidParam = {
	kickedMember: string | GuildMember | User;
	raidId?: number;
	cachedRaidEvent?: RaidEvent;
};
async function kickMemberFromRaid({ kickedMember, cachedRaidEvent, raidId }: KickMemberFromRaidParam) {
	const raidDataBeforeLeave =
		cachedRaidEvent ||
	...
	if (!member || !raidDataBeforeLeave) return;
	...
	if (!rowsUpdated || !raidEvent) return;
	...
	updateRaidMessage({ raidEvent }); // messageid, raid, difficulty
