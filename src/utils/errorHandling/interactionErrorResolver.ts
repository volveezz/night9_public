...
import { IErrorResolver } from "../../interfaces/ErrorResolver.js";
import { pause } from "../general/utilities.js";
import createErrorEmbed from "./createErrorEmbed.js";

export async function interactionErrorResolver({ error, interaction, retryOperation }: IErrorResolver) {
	if (retryOperation) await pause(200);
	...
	try {
		const interactionReply =
			interaction.replied || interaction.deferred ? interaction.followUp(messageOptions) : interaction.reply(messageOptions);
		...
		console.trace(
			`[Error code: 1694] Error during execution of ${
				(interaction as any).customId || (interaction as any).commandName || (interaction as any).name
			} for ${username}\n`,
			error
		);
	} catch (e) {
		if (!retryOperation)
			interactionErrorResolver({ error, interaction, retryOperation: true }).catch((e) =>
				console.error("[Error code: 1695] Received error upon second run of error response")
			);
	}
}
