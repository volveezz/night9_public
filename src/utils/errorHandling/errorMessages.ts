...
function errorMessages(errorType: UserErrors, ...rest: any[]): { embeds: EmbedBuilder[]; components?: any } {
	switch (errorType) {
		...
		case UserErrors.ACTIVITY_MISSING_PERMISSIONS: {
			...
			return {
				embeds: [
					new EmbedBuilder()
						...
						// Rewrite it so it would tell that the user doesn't have permissions to manage the raid OR a LFG post
						// .setDescription("Права для управления рейдом есть у администрации и создателя рейда")
						.setDescription(
							`Права на управление этим событием есть только у его создателя${displayName ? `, **${displayName}**` : ""}`
						)
						.setColor(colors.warning),
				],
			};
			...
			return {
				embeds: [
					new EmbedBuilder()
						...
						.setDescription(
							"Эта ошибка может возникнуть лишь в случае вашего выхода с сервера или нестабильной работой серверов Discord"
						)
						.setColor(colors.error),
				],
			};
			...
			return {
				embeds: [
					new EmbedBuilder()
						...
						.setDescription(
							`Для записи на этот рейд необходимо **${raidRequirement}** закрытий этого рейда, а у вас **${userClears}**`
						),
				],
			};
		}
		...
		case UserErrors.SPECIFIED_MESSAGE_NOT_FOUND: {
			return {
				embeds: [
					new EmbedBuilder()
						.setColor(colors.error)
		...
		case UserErrors.LFG_ACTIVITY_NOT_FOUND: {
		...
		case UserErrors.LFG_ALREADY_JOINED: {
			return {
				embeds: [
					new EmbedBuilder()
						.setColor(colors.serious)
		...
		case UserErrors.LFG_MESSAGE_NOT_FOUND: {
			return {
				embeds: [new EmbedBuilder().setColor(colors.error).setAuthor({ name: "Сообщение не найдено", iconURL: icons.error })],
			};
		}
		case UserErrors.LFG_MISSING_DLC_TO_CREATE_LFG: {
			const requiredDLC: string = rest[0][0];
		...
		case UserErrors.LFG_NOT_AVAILABLE_LFGS_TO_EDIT: {
			return {
				embeds: [
					new EmbedBuilder()
						.setColor(colors.error)
		...
		case UserErrors.LFG_SPECIFY_LFG: {
			const lfgIds: number[] = rest[0];
		...
		case UserErrors.LFG_CANNOT_ADD_BOT: {
			return {
				embeds: [
					new EmbedBuilder()
						.setColor(colors.error)
