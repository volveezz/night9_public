import { DestinyActivityModeType, DestinyPostGameCarnageReportData } from "bungie-api-ts/destiny2/interfaces.js";
import { ButtonBuilder, ButtonStyle, EmbedAuthorOptions, EmbedBuilder, TextChannel } from "discord.js";
...
const hashToImageMap: HashToImageMap = {
	// Ghost of the Deep
...
const checkedPGCRIds = new Set<string>();
const ACTIVITY_LEAVE_TIME = 300; // 5 minutes

let activityChannel: TextChannel | null = null;

/** Activity placeholder names */
...
async function restoreFetchedPGCRs() {
	...
	(await completedActivitiesChannels.messages.fetch({ limit: 15 })).forEach(async (message) => {
		...
		// https://raid.report/pgcr/13127604471
...
function findCorrectedName(hash: number) {
	switch (hash) {
		default:
			return "Засекречено";
	}
...
function findPredefinedName(hash: number) {
	switch (hash) {
		case 4201846671:
...
function getActivityImage(hash: number, manifestImage: string): string {
...
async function getActivityTitle(hash: number, manifestTitle: string): Promise<string> {
...
function getActivityCompletionText(mode: DestinyActivityModeType, fromBeginning: boolean): string {
	return (
...
function getActivityAuthorObject(mode: DestinyActivityModeType, pgcrId: string): EmbedAuthorOptions {
	return {
		name:
			mode === DestinyActivityModeType.Raid
				? "Raid Report"
...
async function logActivityCompletion(pgcrId: string) {
	if (checkedPGCRIds.has(pgcrId)) return;

	const response = await sendApiRequest<DestinyPostGameCarnageReportData>(
		`/Platform/Destiny2/Stats/PostGameCarnageReport/${pgcrId}/`
	).catch((e) => console.error("[Error code: 1072] activityReporter error", pgcrId, e, e.statusCode));
	...
	const isFlawlessCompletion =
		response.activityWasStartedFromBeginning && response.entries.every((player) => player.values.deaths.basic.value === 0);
	...
	const completedUsers = new Map<string, UserActivityStatistics>();
	...
	if (membersMembershipIds.length <= 0) return;
	...
	if (databaseData.length < 1 || clanMembersInActivity < 1) return;

	if (
		(mode === DestinyActivityModeType.Dungeon || mode === DestinyActivityModeType.Raid) &&
		(databaseData.length < membersMembershipIds.length / 2 || clanMembersInActivity < databaseData.length / 2)
	)
		return;
	if (
		mode === DestinyActivityModeType.Story &&
		!((clanMembersInActivity === 1 && membersMembershipIds.length === 1) || clanMembersInActivity > 1)
	)
		return;
	if (mode === DestinyActivityModeType.ScoredNightfall && clanMembersInActivity !== membersMembershipIds.length) return;

	const raidName = mode === DestinyActivityModeType.Raid ? getRaidNameFromHash(referenceId) : null;

	const getFormattedRaidClearsForUser = (bungieId: string): string | null => {
		if (!raidName) return null;

		const databaseDataForUser = databaseData.find((data) => data.bungieId === bungieId);
		if (!databaseDataForUser) return null;

		const totalRaidClearsForUser = completedRaidsData.get(databaseDataForUser.discordId);
		if (!totalRaidClearsForUser) return null;
	...
	const activityModeName = `В ${
		mode === DestinyActivityModeType.Raid ? "рейде" : mode === DestinyActivityModeType.Dungeon ? "подземелье" : "задании"
	}`;

	const raidCharactersIds = new Array<string>();

	for (const [_, { characterId }] of completedUsers) {
		raidCharactersIds.push(characterId);
	}

	const parseActivityUsers = (onlyCompletedUsers: boolean) => {
		let parsedUserCount = 0;

		for (const [
			characterId,
			{ timeInActivity, bungieId, classHash, bungieName, assists, deaths, misc, kills, completed },
		] of completedUsers) {
			...
			parsedUserCount++;
			if (parsedUserCount > 12) break;

			try {
				const hasUserLeftActivity =
					timeInActivity + ACTIVITY_LEAVE_TIME < response.entries[0].values.activityDurationSeconds.basic.value;
				...
				// Additional notes on support weapons the user has used, if any
	...
	if (mode === DestinyActivityModeType.Raid) {
		try {
			...
			if (preciseEncountersTime && preciseEncountersTime.length > 0) {
				const encountersData: CompletedPhase[] = [];
				...
				for (let i = 0; i < phasesArray.length; i++) {
					...
					if (i === 0) {
						encountersData.push({
							...
							// @ts-ignore
							phaseIndex: `${
								encounterData.phaseIndex != 1 && response.activityWasStartedFromBeginning
						...
						const activityEndTime =
							new Date(response.period).getTime() + response.entries[0].values.activityDurationSeconds.basic.value * 1000;
					...
					try {
						embed.addFields({
							...
							value: `${encountersData
								.map((encounter) => {
									return `⁣　⁣${encounter.phaseIndex}. <t:${Math.floor(encounter.start / 1000)}:t> — <t:${Math.floor(
										encounter.end / 1000
									)}:t>, **${convertSeconds(Math.floor((encounter.end - encounter.start) / 1000))}**`;
								})
								.join("\n")}`,
						});
					} catch (error) {
			...
			function getValidatedEncounterTimes(): CompletedPhase[] {
				const completedPhasesForRaidCharacters = new Map<string, CompletedPhase[]>();
				...
				for (const characterId of raidCharactersIds) {
					...
					if (phasesDataPerCharacter) {
						...
						completedPhasesForRaidCharacters.set(characterId, phasesDataPerCharacter); // add to the activityMap
						completedPhases.delete(characterId); // remove from the main map
					}
				}

				// console.debug(`Completed phases", completedPhasesForRaidCharacters);

				const allPhases = new Map<number, CompletedPhase[]>();
				...
				const validatedEncounterTimes: CompletedPhase[] = [];
	...
	if (!activityChannel) activityChannel = await client.getTextChannel(process.env.ACTIVITY_CHANNEL_ID!);
	...
	for (const memberData of databaseData) {
		if (mode === DestinyActivityModeType.Dungeon && clanMembersInActivity > 1) {
			UserActivityData.increment("dungeons", { by: 1, where: { discordId: memberData.discordId } });
			continue;
		} else if (mode !== DestinyActivityModeType.Raid) {
			continue;
		}
		...
		if (!pastCreatedRaid || pastCreatedRaid.time > currentTimeInSeconds) continue;
		...
		const raidCompletionEmbed = new EmbedBuilder()
			...
			.setDescription(
				`Вы создавали рейд ${pastCreatedRaid.id}-${pastCreatedRaid.raid} на <t:${pastCreatedRaid.time}> и сейчас он был завершен\nПодтвердите завершение рейда для удаления набора\n\n[Результаты рейда](https://discord.com/channels/${awaitedMessage.guildId}/${awaitedMessage.channelId}/${awaitedMessage.id})`
			);
		const deleteRaidButton = new ButtonBuilder().setCustomId(RaidButtons.delete).setLabel("Удалить набор").setStyle(ButtonStyle.Danger);
		...
		const raidCompletionNotification = await member
			...
			.catch((e) =>
				console.error("[Error code: 1071] Couldn't send a message to the raid creator. Most likely, their DMs are closed", e)
			);
		...
		setTimeout(async () => {
			...
			if (!raidCompletionNotification) return;
			...
			const updatedEmbed = EmbedBuilder.from(updatedMessage.embeds[0])
				...
				.setDescription(
					`Созданный вами рейд ${pastCreatedRaid.id}-${pastCreatedRaid.raid} на <t:${pastCreatedRaid.time}> был завершен и удален\n\n[Результаты рейда](https://discord.com/channels/${awaitedMessage.guildId}/${awaitedMessage.channelId}/${awaitedMessage.id})`
				);
