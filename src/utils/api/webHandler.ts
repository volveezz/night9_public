import { GetGroupsForMemberResponse } from "bungie-api-ts/groupv2/interfaces.js";
import { UserMembershipData } from "bungie-api-ts/user/interfaces.js";
...
/**
 * Function to validate UUID v4 string
 * @param uuid - UUID string to validate
 * @returns Boolean - If the UUID is valid or not
 */
...
export default async function webHandler(code: string, state: string | string[], res: Response) {
	if (!isValidUUIDv4(state.toString())) return console.error("[Error code: 1958] Received invalid state", state);

	const json = await InitData.findOne({ where: { state } });

	if (!json || !json.discordId) return console.error("[Error code: 1053] No data found", code, state);

	const form = new URLSearchParams();
	form.append("grant_type", "authorization_code");
	form.append("code", code);

	const body: BungieGrantType = (await (
	...
	try {
		const request = await sendApiRequest<UserMembershipData>("/Platform/User/GetMembershipsForCurrentUser/", body.access_token);
		...
		const fetchedData =
			request.destinyMemberships.find((membership) => {
				{
					if (membership.membershipId === request.primaryMembershipId) return membership;
				}
			}) ||
			request.destinyMemberships.find((membership) => {
				{
					if (membership.crossSaveOverride === membership.membershipType) return membership;
				}
			}) ||
			(request.destinyMemberships.length === 1
				? request.destinyMemberships[0]
				: request.destinyMemberships.find((v) => {
						if (v.membershipType === 3) return v;
				  })) ||
			request.destinyMemberships[0];
		...
		if (!created) {
			...
			console.error(
				`[Error code: 1439] User (${json.discordId}) tried to connect already registered bungieId`,
				authData?.discordId,
				authData?.bungieId
			);
			return res.send("<script>location.replace('error.html')</script>");
		...
		const clanResponsePromise = sendApiRequest<GetGroupsForMemberResponse>(
			`/Platform/GroupV2/User/${platform}/${bungieId}/0/1/`,
			body.access_token
		);
		...
		const embed = new EmbedBuilder()
			...
			.setDescription(
				"Для удобства на сервере вы можете указать свой часовой пояс введя команду `/timezone` (</timezone:1055308734794043503>)"
			)
			.setColor(colors.success)
			.addFields(
				{
		...
		if (!member.roles.cache.hasAny(process.env.MEMBER!, process.env.CLANMEMBER!)) givenRoles.push(process.env.MEMBER!);
		if (!member.roles.cache.has(process.env.VERIFIED!)) givenRoles.push(process.env.VERIFIED!);
		if (givenRoles.length > 0) {
			try {
				await member.roles.add(givenRoles, "User registration").then((member) => {
					if (member.roles.cache.has(process.env.NEWBIE!)) member.roles.remove(process.env.NEWBIE!, "User registration");
				});
			} catch (error) {
		...
		async function sendMessageToMember(...buttons: ButtonBuilder[]) {
			try {
				await member.send({
					embeds: [embed],
