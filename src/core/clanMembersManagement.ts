...
import { GroupMember, SearchResultOfGroupMember } from "bungie-api-ts/groupv2";
...
import {
	...
	clanOnline,
	completedPhases,
	recentlyExpiredAuthUsersBungieIds,
	recentlyNotifiedKickedMembers,
	userCharactersId,
} from "../utils/persistence/dataStore.js";
import { AuthData } from "../utils/persistence/sequelizeModels/authData.js";

let lastLoggedErrorCode = PlatformErrorCodes.Success;
let pastInitialLaunch = false;

async function updateClientPresence(errorCode: PlatformErrorCodes) {
	const activities =
...
async function clanMembersManagement(databaseData: AuthData[]) {
	const dataForProcessing = new Array(...databaseData);

	try {
		const request = await sendApiRequest<ServerResponse<SearchResultOfGroupMember>>(
		...
		if (
			...
			updateEndpointStatus("api", PlatformErrorCodes.Success);
			client.user.setPresence({
				activities: [{ name: "API игры восстановлено", type: ActivityType.Custom }],
				status: "online",
			});
			console.info("\x1b[32mBungie API is back online\x1b[0m");
		} else if (errorCode != null && errorCode != getEndpointStatus("api") && errorCode != PlatformErrorCodes.Success) {
		...
		if (!clanList.results || !clanList.results?.length) {
			if (errorCode != null && lastLoggedErrorCode !== errorCode) {
				console.error("[Error code: 1118]", (clanList as any).ErrorStatus, (clanList as any).Message);
				lastLoggedErrorCode = errorCode;
			}
			return;
		}
		...
		await Promise.all(
			clanList.results.map((clanMember) => {
				return processClanMember(clanMember);
			})
		);

		// Process remaining members in databaseData
		await handleClanLeftMembers();

		async function handleClanLeftMembers() {
			await Promise.all(
				dataForProcessing.map(async (member) => {
					if (member.clan === true) {
		...
		async function processClanMember(clanMember: GroupMember) {
			...
			// Handle members that are not registered
			if (index === -1) {
				...
				try {
					...
					if (clanMemberData.member?.groupId === process.env.GROUP_ID!) {
						...
						if (typeof isUserMeetsRequirements === "boolean" && isUserMeetsRequirements === true) {
							...
							console.error(
								"[Error code: 1976]",
								isUserMeetsRequirements,
								memberAuthData.platform,
								memberAuthData.bungieId,
								memberAuthData.accessToken?.length
							);
						}
					}
				} catch (error: any) {
			...
			if (!(memberAuthData.roleCategoriesBits & NightRoleCategory.Triumphs)) return;
			...
			if (!pastInitialLaunch) {
				...
				userCharacterIds?.forEach((characterId) => {
					if (!completedPhases.has(characterId)) return;
					...
					if (!clanMember.isOnline) {
						...
						const interval = setInterval(() => {
							if (completedPhases.get(characterId) !== phasesData) {
								...
								if (completedPhases.has(characterId)) {
									// console.debug(
									// 	`Completed phases data for ${memberAuthData.displayName} was deleted during the interval since the user not online`
									// );
									completedPhases.delete(characterId);
								}
							}
		...
		async function handleNonRegisteredMembers(clanMember: GroupMember) {
			const { membershipType, membershipId } = clanMember.destinyUserInfo;
			if (recentlyExpiredAuthUsersBungieIds.has(membershipId)) return;
...
/**
 * Updates the bot's presence status to reflect the current online members in the clan.
 * @param membersOnline - Number of currently online members.
 * @param totalMembers - Total number of clan members.
 */
function updateBotPresence(membersOnline: number, totalMembers: number): void {
