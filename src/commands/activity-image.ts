...
import { ActivityCustomImage } from "../configs/Buttons.js";
...
const SlashCommand = new Command({
	...
	run: async ({ client, interaction, args }) => {
		...
		if (!raidId && !lfgId)
			throw {
				errorType: UserErrors.LFG_ACTIVITY_NOT_FOUND,
			};
		...
		if (!activityChannel || !activityMessageId) throw {};
		...
		if (!activityMessage || activityMessage.embeds.length != 1) {
			console.error(
				"[Error code: 2023]",
				activityChannel?.name || activityChannel,
				activityMessage?.id || activityMessage,
				activityMessage?.embeds?.length,
				raidId || lfgId
			);
			throw { name: "Произошла непредвиденная ошибка" };
		}

		const SUCCESS_MODAL = new ModalBuilder().setTitle("Укажите ссылки на новые изображения").setCustomId(ActivityCustomImage.Modal);

		const IMAGE_TEXT_MODAL_FIELD = new TextInputBuilder()
			.setCustomId(ActivityCustomImage.ImageField)
		...
		const THUMBNAIL_TEXT_MODAL_FIELD = new TextInputBuilder()
			.setCustomId(ActivityCustomImage.ThumbnailField)
		...
		const newImageUrlLink = modalSubmitInteraction.fields.getTextInputValue(ActivityCustomImage.ImageField);
		const newThumbnailUrlLink = modalSubmitInteraction.fields.getTextInputValue(ActivityCustomImage.ThumbnailField);
