import { ApplicationCommandOption, ApplicationCommandOptionType, EmbedBuilder } from "discord.js";
...
const SlashCommand = new Command({
	...
	run: async ({ client, interaction, args }) => {
		...
		async function deleteCommand(id: string) {
			...
			client.application?.commands
				...
				.catch((e) => {
					...
					} else console.error("[Error code: 1915]", e);
				});
		}

		async function fetchCommands(isGlobal: boolean) {
		...
		async function getCommandList(isGlobal: boolean): Promise<any[]> {
		...
		function createCommandEmbed(commandList: any[], isGlobal: boolean) {
			...
			if (embed.data.fields?.length === 0) embed.setDescription("0 commands");

			return embed;
		}

		async function fetchSpecific(specific_id: string) {
		...
		function formatOptions(
			options: readonly ApplicationCommandOption[] | undefined,
			depth: number = 0,
			lastChild: boolean[] = []
		): string {
			if (!options) return "";
		...
		async function findCommandById(commandId: string) {
