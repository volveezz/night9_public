import { ApplicationCommandOptionType, ButtonBuilder, ButtonStyle, ComponentType, EmbedBuilder, TextChannel } from "discord.js";
...
const SlashCommand = new Command({
	...
	run: async ({ client, interaction, args }) => {
		...
		switch (scriptId) {
			case "logdata": {
				...
				interaction.editReply({
					content: `Currently Running: \`${
						dataForCurrentlyRunning.length === 0 ? "[]" : dataForCurrentlyRunning
					}\`\nCurrent Profiles: \`${dataForCheckedProfiles.length === 0 ? "[]" : dataForCheckedProfiles}\`\nCompleted phases: \`${
						completedPhasesData.length === 0 ? "[]" : completedPhasesData
					}\``,
				});
			...
			// 	return (
			// 		(author === BungieTwitterAuthor.Bungie && guidLowerCase.startsWith("https://twitter.com/bungie/")) ||
			...
			case "test_member_roles": {
				const members = client.getCachedMembers().filter((r) => r.roles.cache.has(process.env.MEMBER!)); // fetching all members

				// Fetch all users from the database with the attribute 'discordId'
				...
				for (const [id, member] of members) {
					...
					if (userDatabaseData) {
						// Determine if the user has required activity to have the roles
						...
						// If user is in the clan or has required activity, allow to have the roles
						if (!userHasActivity) {
							// Filter out the roles that are not supposed to be with non-clan members
							wasInitialCheck &&
			...
			case "checkrole": {
				const members = client
					.getCachedMembers()
					.filter((r) => r.roles.cache.has(process.env.MEMBER!) && !r.roles.cache.has(process.env.VERIFIED!)); // fetching all members
				...
				for (const [id, member] of members) {
					...
					if (hasVerifiedRole) {
						// Check if the user's discordId is in the fetched list
			...
			case "activitytop": {
				const dbData = (await AuthData.findAll({ include: UserActivityData, attributes: ["displayName", "discordId"] })).filter(
					(v) => v.UserActivityData && (v.UserActivityData.messages > 0 || v.UserActivityData.voice > 0)
				);
				...
				const msgEmbed = new EmbedBuilder()
					...
					.setDescription(
						`${messageTop
							.splice(0, 50)
							.map((v, i) => {
								return `${i + 1}. <@${v.discordId}> — ${v.UserActivityData!.messages} ${
									v.UserActivityData!.messages === 1 ? "сообщение" : "сообщений"
								}`;
							})
							.join("\n")}`
					);
			...
			case "roles": {
					...
					value: `\`\`\`fix\nРоли за различные триумфы в игре\`\`\`\n- ${roleData
						...
						.map((data) => {
							const recordDescription =
								roleRequirements[Number(data.triumphRequirement)] ||
				...
				const components = (roleRow: number) => [
					{
