	...
	run: async ({ client, interaction, args }) => {
		...
		async function select() {
			...
			const embed = new EmbedBuilder()
				...
				.setDescription(
					`- ${(userTimezones.get(request.discordId) ?? "Timezone not cached").toString()} | ${request.timezone} <@${
						request.discordId
			...
			if (longOffline.has(request.discordId)) embed.addFields([{ name: "LongOffline", value: "Включен", inline: true }]);
		...
		async function addCase() {
			...
			if (!recordDefinition) throw { name: "Триумф под таким хешем не найден", description: `Hash: ${hash}` };

			const isTitle: boolean = recordDefinition.titleInfo.hasTitle;
			const guildableTitle = isTitle ? (recordDefinition.titleInfo.gildingTrackingRecordHash ? true : false) : false;
			let futureRoleName: string = isTitle
				? guildableTitle
			...
			const category: NightRoleCategory =
				(isTitle ? NightRoleCategory.Titles : args.getInteger("category")) ?? NightRoleCategory.Triumphs;
			...
			collector
				.on("collect", async (collected) => {
					...
					async function confromRoleAdd() {
						const gildedRoles: string[] = [];

						const embed = new EmbedBuilder().setColor(colors.success);

						const rolePosition =
							interaction.guild!.roles.cache.get(
								category === NightRoleCategory.Activity
