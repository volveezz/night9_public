...
import { DestinyActivityDefinition, DestinyAggregateActivityResults, DestinyProfileResponse } from "bungie-api-ts/destiny2/interfaces.js";
...
type ActivityType = { activity: string; activityName: string; clears: number };
...
const SlashCommand = new Command({
	...
	run: async ({ interaction, args }) => {
		...
		const userProfile = await sendApiRequest<DestinyProfileResponse>(profileUrl, authData.accessToken);

		if (!userProfile || !userProfile.characters?.data) throw { name: "Произошла ошибка на стороне Bungie" };

		const activityDefinition = await GetManifest("DestinyActivityDefinition");

		const activityManifest: DestinyActivityDefinition[] =
			args.getBoolean("все-активности") === true
				? activityDefinition
		...
		const activityArray: ActivityType[] = [];
		...
		const characterSet: [Map<string, { clears: number }>, string][] = characterKeys.map((characterKey) => {
		...
		await Promise.all(
			characterKeys.map(async (characterKey, index) => {
				...
				const { activities: freshActivities } = await sendApiRequest<DestinyAggregateActivityResults>(activityStatsUrl, authData);

				if (!freshActivities) throw { name: "Произошла ошибка на стороне Bungie" };
