import { ApplicationCommandOptionType, CommandInteraction, EmbedBuilder, InteractionResponse } from "discord.js";
import { RaidNames, raidSelectionOptions } from "../configs/Raids.js";
...
function generateRaidClears(raidUserData: CompletedRaidsData): string[] {
	const raids = [
		{ id: "se", label: "ГС" },
		{ id: "ce", label: "КК" },
		{ id: "ron", label: "ИК" },
	...
	return raids
		...
		.map(
			(raid) =>
				`${raidUserData[raid.id]}${raidUserData[raid.id + "Master"] > 0 ? `(${raidUserData[raid.id + "Master"]})` : ""} ${raid.label}`
		);
}

function formatRaidUserData(discordId: string): string {
	const raidUserData = completedRaidsData.get(discordId);
	if (!raidUserData) return "нет данных по закрытиям";
...
type SendEmbedParams = {
	...
	selectedRaid: RaidNames;
	interaction: CommandInteraction;
};

async function sendEmbed({ deferredReply, embedIndex, interaction, raidDescription, selectedRaid }: SendEmbedParams) {
...
const SlashCommand = new Command({
	...
	run: async ({ interaction, args }) => {
		...
		const selectedRaid = args.getString("рейд", true) as RaidNames;

		const clanMembers: string[] = [];
		...
		for (const [key, value] of completedRaidsData.entries()) {
			if (value[selectedRaid] + (value[selectedRaid + "Master"] || 0) === 0) {
				...
				if (!member.roles.cache.has(raidRole)) continue;
		...
		const raidClearsList: string[] = allNoviceMembers.map((userId, index) => `${index + 1}. ${formatRaidUserData(userId)}`);
