...
import { ApplicationCommandOptionType, ApplicationCommandType, EmbedBuilder, UserContextMenuCommandInteraction } from "discord.js";
...
const SlashCommand = new Command({
	...
	options: [
		{
			...
			choices: [
				{ name: "рейды", value: `${DestinyActivityModeType.Raid}`, nameLocalizations: { "en-US": "raids", "en-GB": "raids" } },
				{
					name: "подземелья",
					value: `${DestinyActivityModeType.Dungeon}`,
					nameLocalizations: { "en-US": "dungeons", "en-GB": "dungeons" },
				},
				{
					name: "налеты",
					value: `${DestinyActivityModeType.AllStrikes}`,
					nameLocalizations: { "en-US": "strikes", "en-GB": "strikes" },
				},
				{ name: "патруль", value: `${DestinyActivityModeType.Patrol}`, nameLocalizations: { "en-US": "patrol", "en-GB": "patrol" } },
				{ name: "сюжет", value: `${DestinyActivityModeType.Story}`, nameLocalizations: { "en-US": "story", "en-GB": "story" } },
			],
		},
	],
	userContextMenu: {
	...
	run: async ({ args, interaction: slashInteraction, userMenuInteraction }) => {
		if (getEndpointStatus("activity") !== PlatformErrorCodes.Success) {
			throw { errorType: UserErrors.API_UNAVAILABLE };
		}

		const interaction = (userMenuInteraction as UserContextMenuCommandInteraction) || slashInteraction;
		...
		const characterIdList = (
			await sendApiRequest<DestinyHistoricalStatsAccountResult>(
				`/Platform/Destiny2/${platform}/Account/${bungieId}/Stats/?groups=1`,
				accessToken
			)
		).characters.map((characterData) => characterData.characterId);
		...
		async function getCompletedActivties() {
			...
			while (characterIndex < characterIdList.length) {
				...
				const response = await sendApiRequest<DestinyActivityHistoryResults>(
					`/Platform/Destiny2/${platform}/Account/${bungieId}/Character/${characterId}/Stats/Activities/?count=250&mode=${category}&page=${page}`,
					accessToken
				);

				// Update activites array if it exist
		...
		async function summarizeActivities(activities: typeof totalActivitiesCount) {
			const activtitiesTotal: { completed: number; total: number } = { completed: 0, total: 0 };
			// Initialize a dictionary to store the counts of completed and total activities for each referenceId
			const activityCounts: { [referenceId: string]: { completed: number; total: number } } = {};
			// Initialize a dictionary to store the fastest and slowest completed activity times, as well as the instance IDs for those activities, for each referenceId, same for completed activity
			const activityTimes: {
				[referenceId: string]: {
					fastest: number;
					fastestInstanceId: string;
					fastestCompleted: number;
					fastestCompletedInstanceId: string;
					slowest: number;
					slowestInstanceId: string;
					slowestCompleted: number;
					slowestCompletedInstanceId: string;
				};
			} = {};
			// Initialize a dictionary to store the total number of kills, deaths, and time spent in activities for each referenceId
			const activityTotals: { [referenceId: string]: { kills: number; deaths: number; timeSpent: number } } = {};
			// Initialize a dictionary to store the dates and instance IDs for the first and last clear of each referenceId
			const activityDates: {
				[referenceId: string]: {
					firstClear: Date;
					firstClearInstanceId: string;
					firstCompletedClear: Date;
					firstCompletedClearInstanceId: string;
					lastClear: Date;
					lastClearInstanceId: string;
				};
			} = {};
			...
			for (const activity of activities) {
				...
				// Initialize the activity counts for the referenceId if necessary
				if (!activityCounts[name]) {
					activityCounts[name] = { completed: 0, total: 0 };
				}
				// Increment the count of completed and total activities for the referenceId
				if (isCompleted) {
				...
				// Initialize the activity times for the referenceId if necessary
				...
				// Compare the activity time to the stored fastest and slowest activity times for the referenceId, and update the stored values if necessary
				if (
					(!isCompleted && activity.values.activityDurationSeconds.basic.value < activityTimes[name].fastest) ||
				...
				if (
					(!isCompleted && activity.values.activityDurationSeconds.basic.value > activityTimes[name].slowest) ||
				...
				// Initialize the activity totals for the referenceId if necessary
				if (!activityTotals[name]) {
					activityTotals[name] = { kills: 0, deaths: 0, timeSpent: 0 };
				}
				// Add the number of kills, deaths, and time spent in the activity to the stored totals for the referenceId
				...
				// Initialize the activity dates for the referenceId if necessary
				...
				// Compare the completion date of the current activity to the stored first and last clear dates for the referenceId, and update the stored values if necessary
				...
				if (
					new Date(activity.period).getTime() < activityDates[name].firstClear.getTime() ||
			...
			// Return the dictionaries containing the summarized data
		...
		function createActivityObject(category: number): {
			inActivity: string;
			fastest: string;
			fastestCompleted: string;
			link: string;
			slowest: string;
			slowestCompleted: string;
			activity: string;
		} {
			let obj: {
				inActivity: string;
		...
		for (const name of sortedCounts) {
			...
			if (activityCount.total > 0) {
				result.push(
					`Завершено ${activityCount.completed} из ${activityCount.total}｜У: ${activityTotal.kills} С: ${activityTotal.deaths}｜${
						activityText.inActivity
					} ${convertSeconds(activityTotal.timeSpent)}`
				);
			}
			if (
				activityDate.firstClearInstanceId !== "0" ||
				activityDate.lastClearInstanceId !== "0" ||
				activityDate.firstCompletedClearInstanceId !== "0"
			) {
				const { firstCompletedClear, firstClear, firstCompletedClearInstanceId, firstClearInstanceId, lastClearInstanceId, lastClear } =
					activityDate;
				result.push(
					`${
						firstClearInstanceId !== "0" && firstCompletedClear.getTime() > firstClear.getTime()
							? `Впервые <t:${Math.trunc(firstClear.getTime() / 1000)}:R>, [закрытие](${activityText.link}${firstClearInstanceId})｜`
							: ""
					}${
						firstCompletedClearInstanceId !== "0"
							? `1 закрытие <t:${Math.trunc(firstCompletedClear.getTime() / 1000)}:R>, [закрытие](${
									activityText.link
							  }${firstCompletedClearInstanceId})`
							: ""
					}${
						lastClearInstanceId !== firstCompletedClearInstanceId && lastClearInstanceId !== "0"
							? `｜Последнее закрытие <t:${Math.trunc(lastClear.getTime() / 1000)}:R>, [закрытие](${
									activityText.link
							  }${lastClearInstanceId})`
							: ""
					}`
			...
			if (
				activityTime.fastestInstanceId !== "0" ||
				...
				const {
					fastestInstanceId,
					fastestCompleted,
					fastestCompletedInstanceId,
					fastest,
					slowest,
					slowestCompleted,
					slowestCompletedInstanceId,
					slowestInstanceId,
				} = activityTime;
				const joiningText: string[] = [];
				...
				if (fastestCompletedInstanceId !== "0")
					joiningText.push(
						`${activityText.fastestCompleted} [${convertSeconds(fastestCompleted)}](${
							activityText.link
						}${fastestCompletedInstanceId})`
					);
				if (
					...
					joiningText.push(
						`${
							fastestInstanceId !== "0" && fastestInstanceId !== fastestCompletedInstanceId && fastest < fastestCompleted ? "\n" : ""
				...
				if (slowestCompletedInstanceId !== "0" && fastestCompletedInstanceId !== slowestCompletedInstanceId)
					joiningText.push(
						`${activityText.slowestCompleted} [${convertSeconds(slowestCompleted)}](${
							activityText.link
						}${slowestCompletedInstanceId})`
					);
		...
		embed.setColor(colors.success).setAuthor({
			name: `${nameCleaner(targerMember.displayName)} завершил ${filteredActivities.activtitiesTotal.completed} из ${
				filteredActivities.activtitiesTotal.total
