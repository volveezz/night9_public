...
const SlashCommand = new Command({
	...
	run: async ({ client, interaction }) => {
		...
		if (!userData) throw { errorType: UserErrors.DB_USER_NOT_FOUND };
		...
		const collector = (
			await interaction.reply({ embeds: [embed], ephemeral: true, components: addButtonsToMessage(components) })
		...
		collector.on("collect", async (i) => {
			...
			if (userData.displayName.replace("⁣", "").length <= 0) throw { name: "Ваш ник слишком короткий" };
		...
		collector.on("end", (_, reason) => {
			...
			if (reason === "time") interaction.editReply({ embeds: [embed] });
		});
	},
});
