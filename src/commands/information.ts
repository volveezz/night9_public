...
import { GetGroupsForMemberResponse, PlatformErrorCodes } from "bungie-api-ts/groupv2";
...
const SlashCommand = new Command({
	...
	run: async ({ client, interaction: commandInteraction, userMenuInteraction: userInteraction }) => {
		if (getEndpointStatus("account") !== PlatformErrorCodes.Success) {
		...
		// ? interaction.user.id
		// : (interaction as UserContextMenuCommandInteraction | MessageContextMenuCommandInteraction).targetId;

		const targetId = ("targetId" in interaction ? interaction.targetId : interaction.user.id) as string;
		...
		fieldUrls.push(
			`[Bungie.net](https://www.bungie.net/ru/Profile/254/${membershipId})`,
			`[Trials.Report](https://trials.report/report/${platform}/${bungieId})`,
			`[Raid.Report](https://raid.report/${reportPlatform}/${bungieId})`,
			`[Dungeon.Report](https://dungeon.report/${reportPlatform}/${bungieId})`,
			`[Crucible.Report](https://crucible.report/report/${platform}/${bungieId})`,
			`[GM.Report](https://gm.report/${bungieId})`,
			`[Guardian.Report](https://guardian.report/?guardians=${bungieId})`,
			`[DestinyTracker](https://destinytracker.com/destiny-2/profile/${
				platform === 3 ? "steam" : platform === 2 ? "psn" : platform === 1 ? "xbl" : platform === 6 ? "epic" : "stadia"
			}/${bungieId}/overview)`,
			`[WastedOnDestiny](https://wastedondestiny.com/${bungieId})`
		);
			...
			embed.addFields([
				{
					...
					value: `Отправлено сообщений: ${databaseData.UserActivityData.messages}\nВ голосовых каналах: ${convertSeconds(
						databaseData.UserActivityData.voice
					)}`,
					inline: true,
				},
				{
		...
		const fetchProfileAndCharacters = async () => {
			try {
				...
				const response = await sendApiRequest<DestinyProfileResponse>(
					`/Platform/Destiny2/${platform}/Profile/${bungieId}/?components=100,200`
				);
				...
				fieldUrls.push(
					`[Guardian.gg](https://guardian.gg/2/profile/${bungieId}/${data.userInfo.bungieGlobalDisplayName.replace(/\s/g, "%20")})`
				);

				const raceDefinition = await raceDefinitionPromise;

				const characterDataArray: string[] = [];

				for (const characterId in response.characters.data) {
					const character = response.characters.data[characterId];
					const classEmoji =
						character.classHash === 671679327
					...
					characterDataArray.push(
						`${classEmoji}**${raceName}** ${character.light} силы - последний онлайн <t:${Math.floor(
							new Date(character.dateLastPlayed).getTime() / 1000
		...
		const fetchClanData = async () => {
			try {
				const clanBody = await sendApiRequest<GetGroupsForMemberResponse>(`/Platform/GroupV2/User/${platform}/${bungieId}/0/1/`);
				const clanStatus =
					clanBody.results[0]?.group.groupId === process.env.GROUP_ID!
						? "участник клана"
						: clanBody.results[0]
