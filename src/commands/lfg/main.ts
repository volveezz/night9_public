...
const availableRequiredRoles = [
	...
	// 	value: process.env.THE_FINAL_SHAPE_ROLE_ID!,
	// 	nameLocalizations: { "en-GB": "The Final Shape", "en-US": "The Final Shape" },
	// },
	{
		name: "Конец Света",
...
const lfgIdOption: ApplicationCommandOptionData = {
	name: "lfg-id",
...
const SlashCommand = new Command({
	...
	run: async ({ interaction, args }) => {
		...
		let lfgId: number | null = null;

		if (subcommand !== "create") {
			lfgId = await validateAvailableOrInputedLfgId(interaction.user.id, args.getInteger("lfg-id", false));

			if (!lfgId) throw { errorType: UserErrors.LFG_NOT_FOUND, interaction, errorData: [lfgId ?? null] };
		}
		...
		switch (subcommand) {
			case "create": {
				...
				if (convertedTime < Date.now() / 1000 || convertedTime >= 2147483647 || convertedTime < 1000000000) {
					throw { errorType: UserErrors.RAID_TIME_ERROR, errorData: [timeString] };
				}
				...
				if (requiredDLC && !member.roles.cache.has(requiredDLC)) {
					throw { errorType: UserErrors.LFG_MISSING_DLC_TO_CREATE_LFG, errorData: [requiredDLC] };
				}
			...
			case "add": {
				...
				if (user && user.bot) {
					throw { errorType: UserErrors.LFG_CANNOT_ADD_BOT };
				}
