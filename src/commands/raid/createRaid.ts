...
export async function createRaid({
	...
	try {
		...
		const additionalPosition = (guild.channels.cache.get(process.env.RAID_CATEGORY!) as CategoryChannel)?.children?.cache.size || 1;
		...
		const raidEventPromise = RaidEvent.create(
			...
			{ transaction }
		).catch((e) => console.error("[Error code: 2121]", e));
		...
		const raidClearsText = raidClears
			? ` — ${generateRaidCompletionText(raidClears[raidData.raid])}${
					raidClears[raidData.raid + "Master"] ? ` (+**${raidClears[raidData.raid + "Master"]}** на мастере)` : ""
			  }`
			: "";

		const embed = new EmbedBuilder()
			.setTitle(
				`Рейд: ${raidData.raidName}${
					clearRequirement >= 1 ? ` от ${clearRequirement} закрыт${clearRequirement === 1 ? "ия" : "ий"}` : ""
				}`
			)
		...
		// 	{ messageId: message.id, inChannelMessageId: inChannelMessage.id, channelId: privateRaidChannel.id },
		// 	{ transaction, where: { id: raidEvent.id } }
		// );
		...
		deferredReply.then(async (_) => {
			const embed = new EmbedBuilder()
				...
				.setDescription(
					`Канал рейда: <#${privateRaidChannel.id}>, [ссылка на сообщение набора](https://discord.com/channels/${guild.id}/${process
		...
		// await updatePrivateRaidMessage(insertedRaidData[1][0]);
