...
exports.default = (client) => {
    client.on("interactionCreate", (interaction) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        if (!interaction.isButton() ||
            interaction.customId !== "webhandler_clan_request")
            return;
        yield interaction.deferReply({ ephemeral: true });
        if (inviteCd.has(interaction.user.id) ||
            interaction.user.id === ids_1.ownerId) {
        ...
        if (authData.length === 2) {
            if (invitee_clan === true) {
                const embed = new discord_js_1.EmbedBuilder()
                    .setColor("DarkGreen")
                    .setTitle("Вы уже состоите в клане :)")
                    .setTimestamp();
            ...
            try {
                ...
                if (err.error.ErrorCode === 676) {
                    const embed = new discord_js_1.EmbedBuilder()
                        .setColor("DarkGreen")
                        .setTitle("Вы уже участник нашего клана :)");
                    interaction.editReply({ embeds: [embed] });
