...
import { dlcsRoles } from "../base/roles.js";
import { bungieNames } from "../features/full_checker.js";
...
class activitySettings {
    ...
    constructor(activityName, difficulty) {
        var activity = Object.values(CachedDestinyActivityDefinition).find((searchActivity) => {
...
const pvePartyChannel = chnFetcher(ids.pvePartyChnId);
export async function pvePartyHandler(message) {
    ...
    stringArgs?.forEach((args) => {
        ...
        if (args.toLowerCase() === "--f" || args.toLowerCase() === "--frs")
            return (userSettings.ping = dlcsRoles.frs);
        if (args.toLowerCase() === "--sk")
            return (userSettings.ping = dlcsRoles.sk);
        if (args.toLowerCase() === "--bl")
            return (userSettings.ping = dlcsRoles.bl);
        if (args.toLowerCase() === "--anni")
            return (userSettings.ping = dlcsRoles.anni);
        if (args.toLowerCase() === "--twq")
            return (userSettings.ping = dlcsRoles.twq);
        if (args.toLowerCase() === "--lf")
            return (userSettings.ping = dlcsRoles.lf);
    ...
    const voiceChannel = message.member.voice.channel?.type === ChannelType.GuildVoice
        ...
        : await message.guild?.channels.create({
            ...
            permissionOverwrites: pvePartyChannel.permissionOverwrites.cache,
